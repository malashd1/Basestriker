...
export class Game {
    ...
    setPlayerAddress(addr) { this.playerAddr = addr; }
    setShip(id) { this.shopShipId = id; }
    setWeapon(id) { this.shopWeaponId = id; }
    loadLevel(levelId, dailyEpoch = Math.floor(Date.now() / 86_400_000)) {
        this.level = generateLevel(levelId);
        const seed = mixSeed(hashString(this.playerAddr.toLowerCase()), mixSeed(this.level.seedSalt, dailyEpoch));
        ...
        this.player = {
            ...
            wingmenSlots: this.carriedWingmen >= 2 ? [-28, 28] :
    ...
    updatePlayer(inp, dt) {
        ...
        if (inp.fire && p.fireCooldown <= 0) {
            ...
            if (p.weaponMode === 'rocket')
                modeMul = 0.7; // -30% fire rate
            else if (p.weaponMode === 'homing-rocket')
                modeMul = 0.35; // homing-rocket: half of regular rocket = 50% even slower
            else if (p.weaponMode === 'plasma')
                modeMul = 0.85;
            else if (p.weaponMode === 'laser')
                modeMul = 1.15;
    ...
    firePlayer() {
        ...
        if (p.weaponMode !== 'normal') {
            const shoot = (x, y) => {
                if (p.weaponMode === 'laser')
                    this.shootLaser(x, y);
                if (p.weaponMode === 'plasma')
                    this.shootPlasma(x, y);
                if (p.weaponMode === 'rocket')
                    this.shootRocket(x, y);
                if (p.weaponMode === 'homing-rocket')
                    this.shootHomingRocket(x, y);
            };
        ...
        for (const w of p.weapons) {
            const fire = (dx, dy, vx, vy) => this.bullets.push(mkPlayerBullet(p.x + dx, p.y + dy, vx, vy, w.damage, w.color));
            ...
            switch (w.pattern) {
                ...
                case 'spread':
                    ...
                    break;
                case 'laser':
                    this.shootLaser(p.x, p.y - 14);
                    break;
                case 'plasma':
                    this.shootPlasma(p.x, p.y - 14);
                    break;
                case 'homing':
    ...
    advanceWaves(dt) {
        ...
        this.pendingSpawns = this.pendingSpawns.filter((s) => {
            if (this.elapsedMs >= s.at) {
                this.enemies.push(s.e);
                return false;
            }
            return true;
        });
    ...
    updateEnemies(dt) {
        ...
        for (const e of this.enemies) {
            if (e.state === 'enter' || e.state === 'retreat')
                continue;
    ...
    updateBullets(dt) {
        ...
        this.bullets = this.bullets.filter((b) => b.life > 0 && b.x > -10 && b.x < W + 10 && b.y > -10 && b.y < H + 10);
    }
    updateParticles(dt) {
    ...
    applyPowerup(kind) {
        ...
        switch (kind) {
            ...
            case 'rocket':
                ...
                break;
            case 'double':
                p.extraShots = Math.min(2, p.extraShots + 1);
                break;
            case 'rapid':
                p.rapidFire = true;
                break;
            ...
            case 'points':
                p.score += 100;
                this.events.onScoreChange(p.score);
                break;
        }
    ...
    draw() {
        ...
        if (this.bombAnim)
            this.renderer.drawBombAnim(this.bombAnim);
...
function mkPlayerBullet(x, y, vx, vy, damage, color, size = 3) {

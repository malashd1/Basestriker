...
export function currentNetwork() { return _network; }
...
export function walletAddress() { return _address; }
export function walletClient() { return _walletClient; }
async function makeCoinbaseProvider() {
    try {
        const mod = await import('@coinbase/wallet-sdk').catch(() => null);
        if (mod && mod.default) {
            const CoinbaseWalletSDK = mod.default;
...
async function makeWalletConnectProvider() {
    try {
        const projectId = import.meta.env?.VITE_WC_PROJECT_ID || 'a01ec0aae23cc7c2f6fa1f3ef84a7c4f';
...
export async function connect(kindOverride) {
    ...
    if (kind === 'walletconnect') {
        ...
        if (_provider) {
            try {
                ...
                console.warn('[wc] connect failed', e);
            }
        }
    }
    ...
    else if (kind === 'injected' && typeof window.ethereum !== 'undefined') {
        _provider = window.ethereum;
    }
    if (!_provider) {
        // Silent fail when called from auto-reconnect; only alert on user-initiated picker.
        if (!kindOverride)
            alert('Wallet provider unavailable. Try another option.');
        return null;
    }
...
export async function disconnect() {
    ...
    catch { /* not all providers expose this */ }
    _address = null;
    _walletClient = null;
    persistSession(null);
    emit();
...
function readSession() {
    try {
        ...
        if (!blob.address || !blob.kind)
            return null;
        if (Date.now() - (blob.ts ?? 0) > 30 * 86_400_000)
            return null;
        return blob;
    }
    catch {
...
export async function restoreSession() {
    ...
    if (cached.kind === 'injected' && typeof window.ethereum !== 'undefined') {
        try {
            _provider = window.ethereum;
            const accounts = await _provider.request({ method: 'eth_accounts' });
            const fresh = (accounts?.[0] ?? null);

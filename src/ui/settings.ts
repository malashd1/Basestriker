...
interface SettingsCtx {
  ...
  onPurchase?: (priceUsd: number) => void;
}
  ...
  btn.onclick = async () => {
    ...
    try {
      ...
      if (addr) {
        ...
        const net = (currentNetwork?.() as 'base' | 'baseSepolia') ?? DEFAULT_NETWORK;
...
function statsRow(): HTMLElement {
  ...
  const progress = (() => {
    try { return Number(localStorage.getItem('basestriker.progress') ?? '1') || 1; }
    catch { return 1; }
  })();
  const pts = getPoints();

...
function openPanel(panel, render) {
    ...
    wasRunningBeforePanel = game.state === 'running';
...
const game = new Game(canvas, {
    ...
    onPowerupPicked: (kind) => {
        if (kind === 'points')
            earnPoints(PTS_BONUS_LOOT);
    },
});
function updateHUD() {
    if (!game.player)
...
if (currentLevel !== 1)
    btnStart.textContent = `RESUME · LV ${currentLevel}`;
// ── Pause overlay ──────────────────────────────────────────────────
...
addEventListener('keydown', (e) => {
    ...
    if (e.key.toLowerCase() === 'm' || code === 'keym')
        audio.setEnabled(!audio.enabled);
    if (e.key.toLowerCase() === 'n' || code === 'keyn')
...
btnShop.onclick = () => {
    openPanel(shopEl, () => renderShop(shopEl, {
        ...
        equipOne: (id) => {
            switch (id) {
                case 'extra-life':
                    game.carriedLives = (game.carriedLives ?? 1) + 1;
                    return true;
                case 'armor':
                    game.carriedArmor += 1;
                    return true;
                case 'extra-bomb':
                    ...
                    return true;
                case 'rocket':
                    game.carriedWeaponMode = 'rocket';
                    return true;
                case 'homing-rocket':
                    game.carriedWeaponMode = 'homing-rocket';
                    return true;
                case 'wingman':
                    if (game.carriedWingmen >= 2)
                        return false;
                    game.carriedWingmen += 1;
                    return true;
            }
        },
        charge: (priceUsd) => {
            if (priceUsd > 0)
                earnPoints(pointsForUsd(priceUsd));
        },
    }, backToMenu));
};
btnLb.onclick = () => {
...
function renderBackendBadge(ok) {
    const status = document.getElementById('wallet-status');
    let badge = document.getElementById('backend-status');
    if (!badge) {
 ...
 */
function ptsForLevelClear(levelId) {
 ...
 */
async function submitRunToBackend(result, _accepted) {
    if (!walletAddress())
        return;
    try {
        const signed = await postRunToBackend(result);
        try {
            await claimRewards(signed);
        }
        catch (e) {
            console.warn('[run] claim ack skipped:', e?.message ?? e);
        }
    }
    catch (e) {
...
function showClearScreen(result, earned) {
    closeAllPanels();
